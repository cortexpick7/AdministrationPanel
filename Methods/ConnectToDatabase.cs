...
namespace AdministrationPanel.Methods
{
    internal class ConnectToDatabase
    {
        ...
        public void dbAddProduct(string productName, int quantity, double productPrice, string productCategory)
        {
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;Port=5432;Database=ProductDatabase;User Id=postgres;Password=1234");
            NpgsqlCommand comm = new NpgsqlCommand();
            conn.Open();
            comm.Connection = conn;
            comm.CommandType = CommandType.Text;
            string commandText = $"INSERT INTO product (productname, productquantity, productprice, productcategory) VALUES (@name, @quantity, @price, @category)";
        ...
        public void dbAddCategory(string name)
        {
            NpgsqlConnection conn = new NpgsqlConnection("Server=localhost;Port=5432;Database=ProductDatabase;User Id=postgres;Password=1234");
            NpgsqlCommand comm = new NpgsqlCommand();
            conn.Open();
            comm.Connection = conn;
            comm.CommandType = CommandType.Text;
            string commandText = $"INSERT INTO category (categoryname) VALUES (@name)";
            using (var cmd = new NpgsqlCommand(commandText, conn))
            {
